...
//g++ -o tecmp_norm -O3 tecmp_norm.cpp ../src/cmp.cpp  -I../include -I /usr/local/include/SEAL-4.1 -lseal-4.1
...
int main(int argc, char* argv[]){
    ...
    int n = l * m;
    EncryptionParameters parms = tecmp_norm_init(n,l,m,0,0);
    ...
    if(n<=64){
        ...
        cout<<" E(b)[0] = encrypted_op[0] = "<<encrypted_op[0]<<endl;
        ...
        for(int i = 0; i < l ;i++){ plain_op_encode.push_back(rand() % (1 << m)); }    
    ...
    clock_t start=clock();
    Ciphertext result = tecmp_norm(evaluator, gal_keys_server, rlk_server, server_input, client_input, l, m,m_degree, one_zero_zero_cipher);
    ...
    for(int i = 0; i < num_cmps ; i++){
        ...
        //cout<<"i : "<<i<<" : "<<expect_result[i]  <<"  "<<actural_result << endl;
